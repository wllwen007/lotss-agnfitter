...
class MODELSDICT:
    ...
    def construct_dictionaryarray_filtered(self, z, filterdict,path):
        ...
        while t < 10:
            try:
                f = open(path + 'models/GALAXY/bc03_275templates.pickle', 'rb')
                galaxy_object = cPickle.load(f)  
                f.close()
                break
            except:
                if not f.closed:
                    f.close()
                print 'failed to read galaxy_object file, retrying', t
                time.sleep(randint(1, 9))
                t += 1
        else:
            raise Exception('failed to read galaxy_object file too many times')
        ...
        while t < 10:
            try:
                f = open(path + 'models/STARBURST/dalehelou_charyelbaz_v1.pickle', 'rb')
                starburst_object = cPickle.load(f)  
                f.close()
                break
            except:
                if not f.closed:
                    f.close()
                print 'failed to read starburst_object file, retrying', t
                time.sleep(randint(1, 9))
                t += 1
        else:
            raise Exception('failed to read starburst_object file too many times')
        ...
        while t < 10:
            try:
                f = open(path + 'models/BBB/richards.pickle', 'rb')
                bbb_object = cPickle.load(f)  
                f.close()
                break
            except:
                if not f.closed:
                    f.close()
                print 'failed to read bbb_object file, retrying', t
                time.sleep(randint(1, 9))
                t += 1
        else:
            raise Exception('failed to read bbb_object file too many times')
        ...
        while t < 10:
            try:
                f = open(path + 'models/TORUS/silva_v1.pickle', 'rb')
                torus_object = cPickle.load(f)  
                f.close()
                break
            except:
                if not f.closed:
                    f.close()
                print 'failed to read torus_object file, retrying', t
                time.sleep(randint(1, 9))
                t += 1
        else:
            raise Exception('failed to read torus_object file too many times')
